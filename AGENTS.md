...
- **当前版本**: `1.8.3`（唯一来源：`wails.json` 的 `info.productVersion`；前端 `Settings.tsx` 通过 Vite `define` 在构建期注入 `__APP_VERSION__`，**不再硬编码**）
...
| 连接层 | `llm_model` | DeepSeek 模型名，下拉选择 | `deepseek-v4-pro` |
