...
- **当前版本**: `1.7.0`（唯一来源：`wails.json` 的 `info.productVersion`；前端 `Settings.tsx` 通过 Vite `define` 在构建期注入 `__APP_VERSION__`，**不再硬编码**）
...
1. **版本号唯一来源**: `wails.json` 中的 `info.productVersion` 是应用版本的**唯一来源**。前端通过 `frontend/vite.config.ts` 在构建期读取并以 `define` 注入全局常量 `__APP_VERSION__`，`Settings.tsx` 直接引用该常量，**禁止重新硬编码**。构建脚本不再做一致性校验（版本自然一致）。当前版本为 `1.7.0`。
...
| 业务层 | `cache_ttl_hours` | 缓存有效期（小时，1~720，即最长 30 天） | `6` |
