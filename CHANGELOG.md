...
### 新增 (Features)
- **分组删除子菜单**：分组右键「分组删除 ›」拆分为三个选项：
  - 「仅分组」：只删除分组，组内股票保留并移至未分组。
  - 「组内股票」：保留分组，删除组内所有股票。
  - 「分组+股票」：同时删除分组及其内部股票。
  - 三种模式均会在执行前弹窗确认，空分组时「组内股票」自动置灰。

### 优化 (Improvements)
- **分组右键菜单顺序**：「分组合并」菜单项移动到「分组改名」下方。
- **设置面板加宽**：设置下拉面板宽度从 280px 放宽到 360px，减少内容挤压。
- **AI 投研设置排版**：设置 → AI 投研中的「搜索深度」改为标签与下拉框同行显示。

### 修复 (Fixes)
- **修复并发删除导致 watchlist.json 损坏**：批量删除组内股票时，前端由并发调用改为串行调用 `RemoveFromWatchlist`；后端 `SaveWatchlist` 与 `SaveWatchlistGroups` 增加互斥锁，防止并发写文件导致 JSON 损坏。

## [v1.9.0] - 2026-08-30

### 新增 (Features)
- **分组右键「组内添加」**：分组菜单新增「组内添加」，弹出搜索窗口支持按代码/名称/拼音首字母筛选，可批量选择一只股票或多只股票；确认后未在自选的股票会先加入自选，再统一移入目标分组，已在自选的股票会直接移动进该分组。
- **分组右键「分组移动」**：分组菜单新增「分组移动 ›」子菜单，支持「向上 / 向下 / 最上 / 最下」调整分组在列表中的顺序，首尾分组对应方向自动置灰。
- **组内对比新增「半年涨幅」列**：位于「今日涨幅」右侧，数据来自本地日K缓存；缺失时用组内中位数替代，并参与综合得分计算。
- **补齐缺失数据支持半年涨幅**：组内对比点击「补齐缺失数据」时，会同步刷新缺失半年涨幅的股票本地日K缓存。

### 优化 (Improvements)
- **组内对比综合得分权重调整**：营收同比权重 15% → 10%，现金含量权重 10% → 7.5%，新增半年涨幅权重 7.5%；其他指标权重保持不变。
- **组内对比默认排序**：打开组内对比抽屉后默认按综合得分降序排列（#1 在最上方），点击「补齐缺失数据」刷新后仍保持该排序。
- **组内对比抽屉加宽**：抽屉宽度从 1000px 放宽到 1280px，避免「热搜」等列被挤压出现横向滚动条。
- **建议分组忽略持久化**：点击「发现可分组的概念」横幅上的「忽略」后，当天不再请求建议分组数据，次日自动恢复。

### 修复 (Fixes)
- **半年涨幅日期解析**：修复本地日K缓存时间格式为 `20060102` 导致半年涨幅始终无法计算的 bug，并兼容 `2006-01-02` 格式。
...
### 新增 (Features)
- **AI 投研模型列表更新**：设置 → AI 投研中，「模型名称」右侧新增「更新模型」按钮，可调用 DeepSeek / Kimi 服务商 `/models` 接口拉取当前账号可用的最新模型列表；拉取失败时自动回退到内置推荐模型。
- **组内对比排序**：分组视图「组内对比」抽屉支持点击表头按列升序/降序/取消排序，便于快速定位最优/最弱标的。

### 优化 (Improvements)
- **报告品牌统一**：右栏 topbar「财报报告」更名为「透镜报告」；报告封面标题由「{代码} 深度投资分析报告」改为「{股票名称} 财报透镜分析报告」。
- **资金流向卡片默认收起**：个股页面的「当日流向」和「近3日资金流向」默认折叠，点击标题栏可展开，降低首屏信息密度。
- **自选股分组唯一性**：一只股票唯一归属一个分组。右键「移入分组」、跨组拖拽、采纳概念建议建组、新建分组并移入股票时，均会自动将股票从原分组移出。
- **分组右键菜单名称调整**：菜单项统一为「组内对比」、「分组合并 ›」、「分组改名」、「分组删除」。
- **Windows UI 适配**：设置按钮与下拉菜单改为 fixed 定位并兼容自定义标题栏；移除旧主题切换按钮相关样式。
- **风险爬虫进程清理**：简化 Unix 下超时 kill 逻辑，避免进程组残留。

### 文档 (Docs)
- README.md 新增「AI 投研设置指南」，说明 Tavily / DeepSeek / Kimi / Kimi Code 的注册、API Key 申请与配置步骤。
- README.md 新增「自选股分组」说明，补充 `watchlist_groups.json`、`ai_config.json` 等数据文件说明。
- docs/ARCHITECTURE.md 同步 AI 投研配置参数默认值与多 Key 轮询字段。
- docs/BUILD.md 修正 Go 最低版本为 `>= 1.25.0`。

## [v1.8.25] - 2026-08-22

### 新增 (Features)
- **自动更新代理设置**：在「设置 → 关于 → 网络代理」中可配置 HTTP/HTTPS/SOCKS5 代理，解决 GitHub 更新下载超时问题；支持可选的用户名/密码认证。
- **分组合并功能**：分组右键菜单新增「合并分组 ›」，可将当前分组合并到另一个现有分组，并支持在确认弹窗中调整合并后的分组名称。

### 优化 (Improvements)
- **分组右键菜单名称调整**：菜单项统一为「组内股票对比」、「合并分组 ›」、「分组改名」、「删除分组」，语义更清晰。
- **分组 hover 高亮**：鼠标悬停在分组标题上时显示浅蓝色背景，右键操作时可明确辨识目标分组。
- **右键菜单位置自适应**：分组一级菜单与合并子菜单在靠近窗口右/下边缘时自动调整位置，避免被截断。

## [v1.8.22] - 2026-08-21

### 新增 (Features)
- **分组视图全局展开/收起**：自选股分组视图顶部新增可点击的 `›` 箭头，一键展开/收起所有分组，箭头旋转方向与分组展开状态同步。

### 优化 (Improvements)
- **分组工具栏简化**：「+ 新建分组」按钮缩略为 `+` 图标并去掉边框，与全局展开/收起箭头风格统一，顶部工具栏更紧凑。

### 修复 (Fixes)
- **Windows 窗口标题栏**：使用无边框窗口（`Frameless`），消除系统标题栏下方空白菜单栏/留白问题；新增自定义 `TitleBar` 组件。

## [v1.8.21] - 2026-08-19

### 修复 (Fixes)
- **Windows 标题栏**: 强制使用深色主题，消除窗口顶部空白条带。
- **UI 交互**: 采纳「自动发现的概念」建议新建分组后，自动关闭 hover 弹出的成分股列表。

## [v1.8.20] - 2026-08-18

### 新增 (Features)
- **自选股右键菜单增强**：
  - 右键菜单新增「删除自选」，点击后弹出确认对话框，确认后从自选列表删除并同步移除分组。
  - 分组视图下股票右键「移入分组」子菜单底部新增「+ 新建」，可直接将股票移入新建分组。
- **建议分组 hover 提示**：「发现可分组的概念」chip 悬停 500ms 后右侧显示该概念包含的个股列表。

### 优化 (Improvements)
- **右键菜单体验**：移除自选股列表项右侧的 `×` 删除按钮，统一使用右键菜单删除。
- **二级菜单定位优化**：右键「移入分组」二级菜单在靠近窗口底部时自动上移，完整显示内容；修复鼠标移入二级菜单时的跳动问题。
...
### 新增 (Features)
- **自选股概念分组**：
  - 基于东财真实概念板块自动建议分组（IDF 过滤泛概念），一键采纳建组。
  - 支持手动新建空分组、就地改名、删除；分组去重校验。
  - 分组视图下支持组内拖拽排序、跨组拖动、股票右键「移入分组 / 移出分组」（移入时 hover 右侧显示子菜单）。
  - 新增「组内对比」抽屉：横向比较 18步评分、A-Score 风险、营收/净利同比、ROE、毛利率、活跃度、今日涨幅、同花顺热搜排名，每列自动高亮最优/最弱。
  - 平铺/分组视图切换；分组按组内最高活跃度排序，未分组股票始终位于底部。

### 优化 (Improvements)
- **报告阅读体验**：移除财报报告区左侧章节导航目录，保留顶部「跳转章节」下拉框。
- **UI 细节**：分组展开/收起箭头统一为中栏同款 `›` 旋转样式。

## [v1.8.15] - 2026-07-26

### 修复 (Fixes)
- **修复 AI 投研 LLM 输出被 max_tokens 截断导致解析失败**（`ai_researcher/`）
  - 报告类提示词输出较长，旧默认 4096 tokens 经常在 `sources` 中间截断，报 `unexpected end of JSON input`。
  - 新增 `repairTruncatedJSON`：回退到最后一个完整闭合的对象/数组边界并补齐括号，截断时尽力保留已生成板块。
  - 默认 `max_tokens` 4096 → 8192（旧默认值自动迁移）；响应记录 `finish_reason`，截断时输出告警日志。
- **财报报告内容准确性修复**（`analyzer/`）
  - 资金流向表单位错误（元被按万换算成"亿"，出现"主力净流入 1917 亿"），且负值丢失负号；表头摘要与表尾累计口径统一。
  - 「与上次分析对比」的"上次分析时间"显示为股票代码；`AnalysisReport` 新增 `GeneratedAt` 字段持久化，老快照用文件时间兜底。
  - 智能选股条件得分列显示累计分（如 30/15）改为该项得分/满分。
  - 投资检查清单"安全边际"不再永远"暂缺股价"，有实时行情时按 PE/PB 评估（与模块1/6 口径一致）。
  - Engine-B 财务健康分 `/10` → `/100`；ML 融合摘要健康分阈值 6/4 → 60/40（此前恒命中"基本面持续改善"）。
  - 模块7（A-Score）子节编号 8.x → 7.x，技术面子节动态编号，修复撞号与跳号。
  - 可比公司不足 3 家时，模块4 与同行业 A-Score 显示"样本不足"提示并隐藏百分位/排名结论，不再输出误导性对比。

### 优化 (Improvements)
- **报告评分体系统一**：全报告统一 /100 量纲（ML 预测行去除 "-/30"），星级映射统一为 `scoreToStars`，投资检查清单在模块1/12/14 三处数值一致，模块9.3 增加 Engine-D 与 A-Score 口径说明。
- **报告阅读体验**：宽表格横向滚动、报告区「返回顶部」悬浮按钮、左侧章节锚点导航（窄屏自动隐藏）、39 处原生 `alert()` 全面替换为应用内 toast。
- **股吧热帖抽屉**：点击帖子行展开/收起正文，原帖链接改为独立按钮。
- 报告封面年份区间改为正序，报告末尾写入实际生成时间。

### 测试 (Tests)
- 新增 `analyzer/report_fixes_test.go` 8 个用例（金额格式化、diff 时间、选股得分、安全边际、小样本降级、模块编号、资金流向单位与符号）。
- 新增 AI 投研截断修复 4 个用例（板块截断 / sources 截断 / 标量截断 / 配置迁移）。
...
- **修复 AI 投研报告 JSON 多种转义异常**（`ai_researcher/researcher.go`）
  ...
  - 修复字符串值内部未转义双引号（如 `公司与"某医院"签订大单`）导致解析失败的问题。
